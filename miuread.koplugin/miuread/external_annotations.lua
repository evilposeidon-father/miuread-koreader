-- Formal data model and XPointer locator for WeRead annotations projected onto
-- arbitrary local reflowable books. This module never edits the document and
-- has no KOReader UI dependencies.
--
-- Ported from the weread.koplugin "external annotations" feature and adapted
-- to the miuread namespace.
...
function ExternalAnnotations.normalize_catalog(payload, book_id)
    local records = payload
    if type(payload) == "table" and payload.data then
        records = payload.data
    end
    if type(records) ~= "table" then return {} end
    if records.bookId or records.updated or records.chapterInfos or records.chapters then
        records = { records }
    end
...
function ExternalAnnotations.build_thought_items(range_review)
    ...
    local items = {}
    for i, page_review in ipairs(range_review.pageReviews) do
        local review = page_review.review or {}
        ...
        items[#items + 1] = {
            ...
            likes_count = tonumber(page_review.likesCount) or 0,
        }
    end
    return items
end
            ...
            if quote == "" then
                ...
                if result then
                    ...
                    records[#records + 1] = {
                        ...
                        range = range,
                        items = review and ExternalAnnotations.build_thought_items(review) or {},
